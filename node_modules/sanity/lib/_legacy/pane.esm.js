...
import { Box, rem, Flex, Inline, Text, useToast, Card, Stack, ErrorBoundary, Layer, BoundaryElementProvider, MenuDivider, Menu, TextSkeleton, rgba, useElementRect, LayerProvider, _raf2, TextInput, TabList, TabPanel, Code, Heading, focusFirstDescendant, Container, Spinner, usePortal, PortalProvider, Popover, useLayer, useGlobalKeyDown, useClickOutsideEvent, Grid, Button as Button$1, Dialog as Dialog$1, AvatarStack, Badge, DialogProvider, Checkbox, Select } from "@sanity/ui";
import { isValidElement, useMemo, forwardRef, useContext, useCallback, useId, useState, useEffect, useRef, useImperativeHandle, memo, useLayoutEffect, createElement, useDeferredValue, useReducer, Fragment as Fragment$1, lazy } from "react";
...
function DocTitle(props) {
  ...
  return schemaType ? error ? /* @__PURE__ */ jsx(Fragment, { children: t("doc-title.error.text", { errorMessage: error.message }) }) : /* @__PURE__ */ jsx(Fragment, { children: value?.title || /* @__PURE__ */ jsx("span", { style: { color: "var(--card-muted-fg-color)" }, children: t("doc-title.fallback.text") }) }) : /* @__PURE__ */ jsx("code", { children: t("doc-title.unknown-schema-type.text", { schemaType: documentValue._type }) });
}
...
function ReferencePreviewLink(props) {
  const { onClick, type, value } = props, publishedId = getPublishedId(value?._id), documentPresence = useDocumentPresence(publishedId), documentPreviewStore = useDocumentPreviewStore(), { ReferenceChildLink: ReferenceChildLink2 } = usePaneRouter(), Link = useCallback(
    function(linkProps) {
      return /* @__PURE__ */ jsx(
        ReferenceChildLink2,
        {
  ...
  return /* @__PURE__ */ jsxs(Flex, { direction: "column", gap: 4, children: [
    ...
    /* @__PURE__ */ jsx(Card, { radius: 2, shadow: 1, flex: "auto", padding: 2, children: /* @__PURE__ */ jsxs(Flex, { direction: "column", children: [
      internalReferences.totalCount > 0 && /* @__PURE__ */ jsxs(Stack, { as: "ul", marginBottom: 2, space: 2, "data-testid": "internal-references", children: [
        internalReferences?.references.map((item) => /* @__PURE__ */ jsx(Box, { as: "li", children: renderPreviewItem(item) }, item._id)),
...
function useReferringDocuments(documentId) {
  ...
  ).sort(), [crossDatasetReferences?.references]), datasetNames = useMemo(() => Array.from(
    new Set(
  ...
  ).sort(), [crossDatasetReferences?.references]), hasUnknownDatasetNames = useMemo(() => !!crossDatasetReferences?.references.some(
...
function PaneMenuButtonItem(props) {
  ...
  return node.type === "group" ? node.children.length === 0 ? null : node.expanded ? /* @__PURE__ */ jsxs(Fragment, { children: [
    ...
    node.children.map((child, childIndex) => /* @__PURE__ */ jsx(
      PaneMenuButtonItem,
      {
    ...
    /* @__PURE__ */ jsx(
      MenuGroup,
      {
        ...
        text: title,
        children: node.children.map((child, childIndex) => /* @__PURE__ */ jsx(
          PaneMenuButtonItem,
          {
            disabled: disabled || !!node.disabled,
            isAfterGroup: node.children[childIndex - 1]?.type === "group",
...
function PaneHeaderMenuGroupActionButton(props) {
  ...
  return /* @__PURE__ */ jsx(
    MenuButton,
    {
      ...
      id: useId(),
      menu: /* @__PURE__ */ jsx(Menu, { children: node.children.map((child, idx) => /* @__PURE__ */ jsx(
        PaneMenuButtonItem,
        {
          disabled: !!node.disabled,
...
function PaneLayout(props) {
  const { children, minWidth, onCollapse, onExpand, ...restProps } = props, controller = useMemo(() => createPaneLayoutController(), []), [rootElement, setRootElement] = useState(null), width = useElementRect(rootElement)?.width || 0, collapsed = width === void 0 || !minWidth ? void 0 : width < minWidth, [state2, setState] = useState({
...
function PaneHeaderCreateButton({ templateItems }) {
  const schema = useSchema(), templates = useTemplates(), { t } = useTranslation(structureLocaleNamespace), getI18nText = useGetI18nText([...templateItems, ...templates]), [templatePermissions, isTemplatePermissionsLoading] = useTemplatePermissions({
    templateItems
  }), nothingGranted = useMemo(() => !isTemplatePermissionsLoading && templatePermissions?.every((permission) => !permission.granted), [isTemplatePermissionsLoading, templatePermissions]), permissionsById = useMemo(() => templatePermissions ? templatePermissions.reduce(
...
function CommentsProviderWrapper(props) {
  const { children, documentId, documentType } = props, { enabled } = useCommentsEnabled(), { connectionState, onPathOpen, inspector, openInspector } = useDocumentPane(), { params, setParams, createPathWithParams } = usePaneRouter(), selectedCommentId = params?.comment, paramsRef = useRef(params);
  useLayoutEffect(() => {
...
const IGNORE_OPS = ["patch", "commit"], DocumentOperationResults = memo(function() {
  const { push: pushToast } = useToast(), { documentId, documentType, value: documentPaneValue } = useDocumentPane(), documentTitleInfo = useDocumentTitle(), titleError = documentTitleInfo.error, event = useDocumentOperationEvent(documentId, documentType), prevEvent = useRef(event), paneRouter = usePaneRouter(), { t } = useTranslation(structureLocaleNamespace), documentTitleBase = useMemo(() => !documentTitleInfo.title && !titleError && !IGNORE_OPS.includes(event?.op) && typeof documentPaneValue.title == "string" && event?.type === "success" ? documentPaneValue.title : documentTitleInfo.title, [documentTitleInfo.title, titleError, event, documentPaneValue.title]) || t("panes.document-operation-results.operation-undefined-title"), documentTitle = documentTitleBase.length > 25 ? `${documentTitleBase.slice(0, 25)}...` : documentTitleBase;
  return useEffect(() => {
...
const ReferenceChangedBanner = memo(() => {
  const documentPreviewStore = useDocumentPreviewStore(), { params, groupIndex, routerPanesState, replaceCurrent, BackLink: BackLink2 } = usePaneRouter(), routerReferenceId = routerPanesState[groupIndex]?.[0].id, parentSibling = routerPanesState[groupIndex - 1]?.[0], parentId = parentSibling?.id, hasHistoryOpen = !!parentSibling?.params?.rev, parentRefPath = useMemo(() => params?.parentRefPath && fromString(params.parentRefPath) || null, [params?.parentRefPath]), { t } = useTranslation(structureLocaleNamespace), referenceInfoObservable = useMemo(() => {
  ...
  return (
    ...
    referenceInfo.result?.refValue === routerReferenceId ? null : /* @__PURE__ */ jsx(
      Banner,
      {
        ...
        content: /* @__PURE__ */ jsx(Text, { size: 1, weight: "medium", children: referenceInfo.result?.refValue ? t("banners.reference-changed-banner.reason-changed.text") : t("banners.reference-changed-banner.reason-removed.text") }),
...
`, FormHeader = ({ documentId, schemaType, title }) => {
  ...
  return schemaType.__experimental_formPreviewTitle === !1 ? null : /* @__PURE__ */ jsxs(TitleContainer, { marginBottom: 6, space: 4, children: [
    !isSingleton && /* @__PURE__ */ jsx(Text, { muted: !0, size: 1, children: schemaType.title ?? schemaType.name }),
    /* @__PURE__ */ jsx(Heading, { as: "h2", "data-heading": !0, muted: !title, "data-testid": "document-panel-document-title", children: title ?? t("document-view.form-view.form-title-fallback") })
  ] });
}, LONG_ENOUGH_BUT_NOT_TOO_LONG = 1e3 * 60 * 60 * 24 * 24;
function useConditionalToast(params) {
  const toast = useToast();
  useEffect(() => (params.enabled && toast.push({ ...params, duration: LONG_ENOUGH_BUT_NOT_TOO_LONG }), () => {
...
const preventDefault = (ev) => ev.preventDefault(), FormView = forwardRef(function(props, ref) {
  const { hidden, margins } = props, {
    ...
    openPath
  } = useDocumentPane(), documentStore = useDocumentStore(), presence = useDocumentPresence(documentId), { title } = useDocumentTitle(), patchChannel = useMemo(() => createPatchChannel(), []), isLocked = editState?.transactionSyncLock?.enabled, { t } = useTranslation(structureLocaleNamespace), conditionalToastParams = useMemo(
    () => ({
      id: "sync-lock",
      status: "warning",
      enabled: isLocked,
...
function ConfirmDialogContent(props) {
  const { dialog } = props, {
    ...
    onConfirm,
    tone
  } = dialog, { t } = useTranslation(structureLocaleNamespace), { isTopLayer } = useLayer(), ref = useRef(null), handleGlobalKeyDown = useCallback(
    (event) => {
      event.key === "Escape" && isTopLayer && onCancel();
    },
    [isTopLayer, onCancel]
  );
  return useGlobalKeyDown(handleGlobalKeyDown), useClickOutsideEvent(isTopLayer && onCancel, () => [ref.current]), /* @__PURE__ */ jsxs(Flex, { direction: "column", ref, style: { minWidth: 304, maxWidth: 400 }, children: [
...
function PopoverDialogContent(props) {
  const { dialog } = props, { content, onClose } = dialog, { isTopLayer } = useLayer(), ref = useRef(null), handleGlobalKeyDown = useCallback(
    (event) => {
      event.key === "Escape" && isTopLayer && onClose();
    },
    [isTopLayer, onClose]
  );
  return useGlobalKeyDown(handleGlobalKeyDown), useClickOutsideEvent(isTopLayer && onClose, () => [ref.current]), /* @__PURE__ */ jsx("div", { ref, children: content });
}
function DocumentActionPortalProvider(props) {
  const { children } = props, { element, elements } = usePortal(), portalElement = elements?.[DOCUMENT_PANEL_PORTAL_ELEMENT] || element;
...
function ActionStateDialog(props) {
  ...
  if (dialog.type === "custom")
    return /* @__PURE__ */ jsx(DocumentActionPortalProvider, { children: dialog?.component });
...
const ActionDialogWrapper = memo(function({
  actionStates,
  children,
  referenceElement
}) {
  const [actionIndex, setActionIndex] = useState(-1), currentAction = useMemo(() => actionStates[actionIndex], [actionIndex, actionStates]), handleAction = useCallback((idx) => {
    setActionIndex(idx);
  }, []), result = useMemo(() => children({ handleAction }), [children, handleAction]);
  return /* @__PURE__ */ jsxs(Fragment, { children: [
...
function ActionMenuButton(props) {
  const { actionStates, disabled } = props, idPrefix = useId(), [referenceElement, setReferenceElement] = useState(null), popoverProps = useMemo(
    ...
    []
  ), { t } = useTranslation(structureLocaleNamespace), renderActionDialog = useCallback(
    ({ handleAction }) => /* @__PURE__ */ jsx(
      MenuButton,
      {
  ...
  return /* @__PURE__ */ jsx(ActionDialogWrapper, { actionStates, referenceElement, children: renderActionDialog });
}
...
const DISABLED_REASON_KEY$2 = {
  ...
  return useMemo(() => !published || liveEdit ? null : !isPermissionsLoading && !permissions?.granted ? {
    tone: "critical",
    icon: ResetIcon,
    disabled: !0,
...
const HistoryRestoreAction = ({ id, type, revision, onComplete }) => {
  ...
  return useMemo(() => isRevisionLatest ? null : {
...
const PublishAction = (props) => {
  const { id, type, liveEdit, draft, published } = props, [publishState, setPublishState] = useState(null), { publish } = useDocumentOperation(id, type), validationStatus = useValidationStatus(id, type), syncState = useSyncState(id, type), { changesOpen, onHistoryOpen, documentId, documentType } = useDocumentPane(), editState = useEditState(documentId, documentType), { t } = useTranslation(structureLocaleNamespace), revision = (editState?.draft || editState?.published || {})._rev, hasValidationErrors = validationStatus.validation.some(isValidationErrorMarker), [publishScheduled, setPublishScheduled] = useState(!1), isSyncing = syncState.isSyncing, isValidating = validationStatus.isValidating, [permissions, isPermissionsLoading] = useDocumentPairPermissions({
    id,
    type,
    permission: "publish"
...
const DocumentStatusBarActionsInner = memo(function(props) {
  ...
  const { actions: allActions, connectionState, documentId, editState } = useDocumentPane(), actions = useMemo(
    ...
    [allActions]
  ), renderDocumentStatusBarActions = useCallback(
    ({ states }) => /* @__PURE__ */ jsx(
  ...
  const { actions, connectionState, editState, timelineStore } = useDocumentPane(), revision = useTimelineSelector(timelineStore, (state2) => state2.revTime)?.id || "", disabled = (editState?.draft || editState?.published || {})._rev === revision, actionProps = useMemo(() => ({ ...editState || {}, revision }), [editState, revision]), historyActions = useMemo(() => (actions ?? []).filter(isRestoreAction).slice(-1), [actions]), renderDocumentStatusBarActions = useCallback(
    ({ states }) => /* @__PURE__ */ jsx(
      DocumentStatusBarActionsInner,
      {
...
function DocumentHeaderTabs() {
  ...
  return /* @__PURE__ */ jsx(TabList, { space: 1, children: views2.map((view, index) => /* @__PURE__ */ jsx(
    DocumentHeaderTab,
    {
      icon: view.icon,
...
function DocumentHeaderTitle() {
  ...
  return connectionState === "connecting" ? /* @__PURE__ */ jsx(Fragment, {}) : title ? /* @__PURE__ */ jsx(Fragment, { children: title }) : documentValue ? error ? /* @__PURE__ */ jsx(Fragment, { children: t("panes.document-header-title.error.text", { error: error.message }) }) : /* @__PURE__ */ jsx(Fragment, { children: value?.title || /* @__PURE__ */ jsx("span", { style: { color: "var(--card-muted-fg-color)" }, children: t("panes.document-header-title.untitled.text") }) }) : /* @__PURE__ */ jsx(Fragment, { children: t("panes.document-header-title.new.text", {
...
const DocumentPanelHeader = memo(
...
), DocumentPanelHeaderActionDialogDeferred = memo(
  function(props) {
    const { setReferenceElement, referenceElement, contextMenuNodes } = props, states = useDeferredValue(props.states);
    return /* @__PURE__ */ jsx(
  ...
  const { states, setReferenceElement, contextMenuNodes, referenceElement } = props, renderActionDialog = useCallback(
    ({ handleAction }) => /* @__PURE__ */ jsx("div", { ref: setReferenceElement, children: /* @__PURE__ */ jsx(
  ...
  return /* @__PURE__ */ jsx(ActionDialogWrapper, { actionStates: states, referenceElement, children: renderActionDialog });
}), BADGE_TONES = {
  primary: "primary",
  success: "positive",
  warning: "caution",
  danger: "critical"
}, DocumentBadgesInner = memo(function({ states }) {
  ...
  return /* @__PURE__ */ jsx(DocumentBadgesInner, { states });
});
function DocumentBadges() {
  const { badges, editState } = useDocumentPane(), renderDocumentBadges = useCallback(({ states }) => /* @__PURE__ */ jsx(DocumentBadgesDeferred, { states }), []);
  return !editState || !badges ? null : /* @__PURE__ */ jsx(RenderBadgeCollectionState, { badges, badgeProps: editState, children: renderDocumentBadges });
}
...
const STATUS_DICTIONARY = {
  ...
  return /* @__PURE__ */ jsxs(Flex, { align: "center", gap: 2, "data-testid": "pane-footer-document-status-pulse", children: [
...
function DocumentStatusLine({ singleLine }) {
  const { documentId, documentType, editState, value } = useDocumentPane(), [status, setStatus] = useState(null), syncState = useSyncState(documentId, documentType), lastUpdated = value?._updatedAt;
  return useEffect(() => {
    ...
    Tooltip,
    {
      content: /* @__PURE__ */ jsx(DocumentStatus, { absoluteDate: !0, draft: editState?.draft, published: editState?.published }),
      placement: "top",
      children: /* @__PURE__ */ jsxs(Flex, { align: "center", gap: 3, children: [
        /* @__PURE__ */ jsx(DocumentStatusIndicator, { draft: editState?.draft, published: editState?.published }),
        /* @__PURE__ */ jsx(
          DocumentStatus,
          {
...
const KeyboardShortcutResponder = memo(function(props) {
  ...
  (props) => {
    const { actionsBoxElement, as = "div", children, ...rest } = props, { actions, editState } = useDocumentPane(), [activeIndex, setActiveIndex] = useState(-1), onActionStart = useCallback((idx) => {
      ...
      [editState]
    ), renderDocumentActionShortcuts = useCallback(
      ({ states }) => /* @__PURE__ */ jsx(
    ...
    return !actionProps || !actions ? null : /* @__PURE__ */ jsx(RenderActionCollectionState, { actionProps, actions, children: renderDocumentActionShortcuts });
  }
);
...
const DocumentPaneProvider = memo((props) => {
  const { children, index, pane: pane2, paneKey, onFocusPath } = props, schema = useSchema(), templates = useTemplates(), { setDocumentMeta } = useCopyPaste(), {
    __internal_tasks,
    document: {
    ...
    templateName,
    templateParams
  }), initialValue = useUnique(initialValueRaw), { patch } = useDocumentOperation(documentId, documentType), editState = useEditState(documentId, documentType), { validation: validationRaw } = useValidationStatus(documentId, documentType), connectionState = useConnectionState(documentId, documentType), schemaType = schema.get(documentType), value = editState?.draft || editState?.published || initialValue.value, [isDeleting, setIsDeleting] = useState(!1), actions = useMemo(
  ...
  ), focusPathRef = useRef([]), activeViewId = params.view || views2[0] && views2[0].id || null, [timelineMode, setTimelineMode] = useState("closed"), [timelineError, setTimelineError] = useState(null), timelineStore = useTimelineStore({
    documentId,
    documentType,
    onError: setTimelineError,
    rev: params.rev,
    since: params.since
  }), onOlderRevision = useTimelineSelector(timelineStore, (state2) => state2.onOlderRevision), revTime = useTimelineSelector(timelineStore, (state2) => state2.revTime), sinceAttributes = useTimelineSelector(timelineStore, (state2) => state2.sinceAttributes), timelineDisplayed = useTimelineSelector(timelineStore, (state2) => state2.timelineDisplayed), timelineReady = useTimelineSelector(timelineStore, (state2) => state2.timelineReady), isPristine = useTimelineSelector(timelineStore, (state2) => state2.isPristine), isDeleted = useMemo(() => timelineReady ? !editState?.draft && !editState?.published && !isPristine : !1, [editState?.draft, editState?.published, isPristine, timelineReady]), previewUrl = usePreviewUrl(value), [presence, setPresence] = useState([]);
  useEffect(() => {
  ...
  const currentInspector = inspectors2?.find((i) => i.name === inspectorName), resolvedChangesInspector = inspectors2.find((i) => i.name === HISTORY_INSPECTOR_NAME), changesOpen = currentInspector?.name === HISTORY_INSPECTOR_NAME, { t } = useTranslation(structureLocaleNamespace), inspectOpen = params.inspect === "on", compareValue = changesOpen ? sinceAttributes : editState?.published || null, fieldActions = useMemo(
    ...
    (blurredPath) => {
      disableBlurRef.current || (setFocusPath(EMPTY_ARRAY$3), focusPathRef.current !== EMPTY_ARRAY$3 && (focusPathRef.current = EMPTY_ARRAY$3, onFocusPath?.(EMPTY_ARRAY$3)));
    },
  ...
  const handleChange = useCallback((event) => patchRef.current(event), []), closeInspector = useCallback(
  ...
  ), telemetry = useTelemetry(), handleMenuAction = useCallback(
    (item) => {
    ...
    []
  ), requiredPermission = value._createdAt ? "update" : "create", liveEdit = !!schemaType?.liveEdit, docId = value._id ? value._id : "dummy-id", docPermissionsInput = useMemo(() => ({
    ...value,
    _id: liveEdit ? getPublishedId(docId) : getDraftId(docId)
  }), [liveEdit, value, docId]), [permissions, isPermissionsLoading] = useDocumentValuePermissions({
    document: docPermissionsInput,
    permission: requiredPermission
  }), isNonExistent = !value?._id, readOnly = useMemo(() => {
    const hasNoPermission = !isPermissionsLoading && !permissions?.granted, updateActionDisabled = !isActionEnabled(schemaType, "update"), createActionDisabled = isNonExistent && !isActionEnabled(schemaType, "create"), reconnecting = connectionState === "reconnecting", isLocked = editState.transactionSyncLock?.enabled;
  ...
  const setOpenPath = useCallback(
    ...
    (nextFocusPath, payload) => {
      setFocusPath(nextFocusPath), deepEquals(focusPathRef.current, nextFocusPath) || (setOpenPath(nextFocusPath.slice(0, -1)), focusPathRef.current = nextFocusPath, onFocusPath?.(nextFocusPath)), updatePresenceThrottled(nextFocusPath, payload);
    },
  ...
  return useEffect(() => {
    if (ready && params.path) {
      const { path, ...restParams } = params, pathFromUrl = resolveKeyedPath(formStateRef.current?.value, fromString(path));
      disableBlurRef.current = !0, deepEquals(focusPathRef.current, pathFromUrl) || (setFocusPath(pathFromUrl), setOpenPath(pathFromUrl), focusPathRef.current = pathFromUrl, onFocusPath?.(pathFromUrl));
...
const DocumentPane = memo(function(props) {
  ...
  return /* @__PURE__ */ jsx(SourceProvider, { name: props.pane.source || parentSourceName, children: /* @__PURE__ */ jsx(CopyPasteProvider, { children: /* @__PURE__ */ jsx(DocumentPaneInner, { ...props }) }) });
});
...
function useDocumentList(opts) {
  const { filter: filter2, params: paramsProp, sortOrder, searchQuery, apiVersion } = opts, client = useClient({
  ...
  }), { enableLegacySearch = !1 } = useWorkspace().search, schema = useSchema(), maxFieldDepth = useSearchMaxFieldDepth(), [resultState, setResult] = useState(INITIAL_STATE), { onRetry, error, result } = resultState, documents = result?.documents, items = useMemo(
    ...
    [filter2, paramsProp]
  ), isLoading = result === null && !error, hasMaxItems = documents?.length === FULL_LIST_LIMIT, onListChange = useCallback(() => {
...
function ColumnsControl({ table }) {
  ...
  return /* @__PURE__ */ jsx(
    MenuButton,
    {
      button: /* @__PURE__ */ jsx(Button, { mode: "bleed", text: t("sheet-list.edit-columns") }),
      id: "columns-control",
      menu: /* @__PURE__ */ jsx(Menu, { padding: 3, paddingTop: 4, style: { width: 240 }, children: /* @__PURE__ */ jsxs(Flex, { direction: "column", height: "fill", gap: 3, children: [
        ...
        /* @__PURE__ */ jsx(Flex, { style: { flex: "1 1 auto", maxHeight: 320, overflowY: "scroll" }, children: /* @__PURE__ */ jsx(Stack, { children: table.getAllLeafColumns().filter((column) => column.getCanHide()).map((column) => /* @__PURE__ */ jsxs(Flex, { marginY: 2, align: "center", children: [
          /* @__PURE__ */ jsx(
            Checkbox,
            {
          ...
          /* @__PURE__ */ jsx(Box, { flex: 1, paddingLeft: 3, children: /* @__PURE__ */ jsx(Text, { size: 1, children: /* @__PURE__ */ jsx("label", { htmlFor: `col-visibility-${column.id}`, children: column.columnDef.header?.toString() }) }) })
        ] }, column.id)) }) }),
        /* @__PURE__ */ jsx(
          Button,
          {
...
const addSelectedStateToMenuItems = (options) => {
  ...
  return menuItems?.map((item) => item.params?.layout ? {
    ...
    ...item,
    selected: isEqual(sortOrderRaw?.by, item?.params?.by || EMPTY_ARRAY$3)
  } : { ...item, selected: !1 });
};
...
const PaneContainer = memo(function(props) {
  const { index, isSelected, pane: pane2, paneKey } = props, { name: parentSourceName } = useSource(), {
    ...
    defaultLayout
  ), { title } = useI18nText(pane2), defaultSortOrder = useMemo(() => defaultOrdering?.length > 0 ? { by: defaultOrdering } : DEFAULT_ORDERING, [defaultOrdering]), [sortOrderRaw, setSortOrder] = useStructureToolSetting(
    "sort-order",
    typeName ?? pane2.id,
    //pane.id for anything that is not documentTypeList
    defaultSortOrder
...
function menuItemsFromInitialValueTemplateItems(context, templateItems) {
  ...
  return templateItems.map((item) => {
    ...
    let builder = new MenuItemBuilder(context).title(title).icon(template && template.icon || schemaType?.icon || AddIcon).intent({ type: "create", params: intentParams });
