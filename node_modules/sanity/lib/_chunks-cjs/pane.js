...
function DocTitle(props) {
  ...
  return schemaType ? error ? /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children: t("doc-title.error.text", { errorMessage: error.message }) }) : /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children: value?.title || /* @__PURE__ */ jsxRuntime.jsx("span", { style: { color: "var(--card-muted-fg-color)" }, children: t("doc-title.fallback.text") }) }) : /* @__PURE__ */ jsxRuntime.jsx("code", { children: t("doc-title.unknown-schema-type.text", { schemaType: documentValue._type }) });
}
...
function ReferencePreviewLink(props) {
  const { onClick, type, value } = props, publishedId = sanity.getPublishedId(value?._id), documentPresence = sanity.useDocumentPresence(publishedId), documentPreviewStore = sanity.useDocumentPreviewStore(), { ReferenceChildLink: ReferenceChildLink2 } = usePaneRouter(), Link = React.useCallback(
    function(linkProps) {
      return /* @__PURE__ */ jsxRuntime.jsx(
        ReferenceChildLink2,
        {
    ...
    [ReferenceChildLink2, type?.name, value?._id]
  );
  return /* @__PURE__ */ jsxRuntime.jsx(sanity.PreviewCard, { __unstable_focusRing: !0, as: Link, "data-as": "a", onClick, radius: 2, children: /* @__PURE__ */ jsxRuntime.jsx(
    PaneItemPreview,
    {
      ...
      icon: type?.icon,
      layout: "compact",
      presence: documentPresence?.length > 0 ? documentPresence : EMPTY_ARRAY$2,
      schemaType: type,
      value
    }
  ) });
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { direction: "column", gap: 4, children: [
    ...
    /* @__PURE__ */ jsxRuntime.jsx(ui.Card, { radius: 2, shadow: 1, flex: "auto", padding: 2, children: /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { direction: "column", children: [
      internalReferences.totalCount > 0 && /* @__PURE__ */ jsxRuntime.jsxs(ui.Stack, { as: "ul", marginBottom: 2, space: 2, "data-testid": "internal-references", children: [
        internalReferences?.references.map((item) => /* @__PURE__ */ jsxRuntime.jsx(ui.Box, { as: "li", children: renderPreviewItem(item) }, item._id)),
...
function useReferringDocuments(documentId) {
  const versionedClient = sanity.useClient(sanity.DEFAULT_STUDIO_CLIENT_OPTIONS), documentStore = sanity.useDocumentStore(), publishedId = sanity.getPublishedId(documentId), [internalReferences, isInternalReferencesLoading] = useInternalReferences(
    ...
    new Set(
      crossDatasetReferences?.references.map((crossDatasetReference) => crossDatasetReference.projectId).filter(Boolean)
    )
  ).sort(), [crossDatasetReferences?.references]), datasetNames = React.useMemo(() => Array.from(
    new Set(
      crossDatasetReferences?.references.map((crossDatasetReference) => crossDatasetReference?.datasetName || "").filter((datasetName) => !!datasetName && datasetName !== "")
    )
  ).sort(), [crossDatasetReferences?.references]), hasUnknownDatasetNames = React.useMemo(() => !!crossDatasetReferences?.references.some(
...
`, Pane = React.forwardRef(function(props, forwardedRef) {
  ...
  const handleCollapse = React.useCallback(() => {
    ...
    if (!(layoutCollapsed && isLast))
      return isLast ? maxWidthProp ? currentMaxWidth ?? maxWidthProp : void 0 : currentMaxWidth ?? maxWidthProp;
...
function PaneMenuButtonItem(props) {
  ...
  return node.type === "group" ? node.children.length === 0 ? null : node.expanded ? /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    ...
    node.children.map((child, childIndex) => /* @__PURE__ */ jsxRuntime.jsx(
      PaneMenuButtonItem,
      {
    ...
    /* @__PURE__ */ jsxRuntime.jsx(
      TooltipDelayGroupProvider.MenuGroup,
      {
        ...
        text: title,
        children: node.children.map((child, childIndex) => /* @__PURE__ */ jsxRuntime.jsx(
          PaneMenuButtonItem,
          {
            disabled: disabled || !!node.disabled,
            isAfterGroup: node.children[childIndex - 1]?.type === "group",
...
`, TabsBox = styledComponents.styled(ui.Box)(({ theme }) => {
  ...
  const { actions, backButton, border, contentAfter, loading, subActions, tabs, tabIndex, title } = props, { collapse, collapsed, expand, rootElement: paneElement } = usePane(), paneRect = ui.useElementRect(paneElement || null), layoutStyle = React.useMemo(
    () => ({
      width: collapsed ? paneRect?.height || window.innerHeight : void 0
    }),
    [collapsed, paneRect]
    ...
    TooltipDelayGroupProvider.MenuButton,
    {
      button: /* @__PURE__ */ jsxRuntime.jsx(
        TooltipDelayGroupProvider.Button,
        {
          ...
          icon: node.icon ?? icons.UnknownIcon,
          label: title,
          mode: "bleed",
      ...
      id: React.useId(),
      menu: /* @__PURE__ */ jsxRuntime.jsx(ui.Menu, { children: node.children.map((child, idx) => /* @__PURE__ */ jsxRuntime.jsx(
        PaneMenuButtonItem,
        {
          disabled: !!node.disabled,
...
function PaneLayout(props) {
  const { children, minWidth, onCollapse, onExpand, ...restProps } = props, controller = React.useMemo(() => createPaneLayoutController(), []), [rootElement, setRootElement] = React.useState(null), width = ui.useElementRect(rootElement)?.width || 0, collapsed = width === void 0 || !minWidth ? void 0 : width < minWidth, [state2, setState] = React.useState({
...
function PaneHeaderCreateButton({ templateItems }) {
  const schema = sanity.useSchema(), templates = sanity.useTemplates(), { t } = sanity.useTranslation(structureLocaleNamespace), getI18nText = sanity.useGetI18nText([...templateItems, ...templates]), [templatePermissions, isTemplatePermissionsLoading] = sanity.useTemplatePermissions({
    templateItems
  }), nothingGranted = React.useMemo(() => !isTemplatePermissionsLoading && templatePermissions?.every((permission) => !permission.granted), [isTemplatePermissionsLoading, templatePermissions]), permissionsById = React.useMemo(() => templatePermissions ? templatePermissions.reduce(
  ...
  if (templateItems.length === 1) {
    const firstItem = templateItems[0], disabled = !permissionsById[firstItem.id]?.granted, intent = getIntent(schema, templates, firstItem);
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    TooltipDelayGroupProvider.MenuButton,
    {
      ...
      menu: /* @__PURE__ */ jsxRuntime.jsx(ui.Menu, { children: templateItems.map((item, itemIndex) => {
        const disabled = !permissionsById[item.id]?.granted, intent = getIntent(schema, templates, item), template = templates.find((i) => i.id === item.templateId);
...
const hashObject = (value) => {
  ...
  const {
    ...
    () => resolveMenuNodes({
      ...
      menuItemGroups,
      menuItems: menuItems.filter((item) => item.intent?.type !== "create")
    }),
...
function CommentsProviderWrapper(props) {
  const { children, documentId, documentType } = props, { enabled } = sanity.useCommentsEnabled(), { connectionState, onPathOpen, inspector, openInspector } = useDocumentPane(), { params, setParams, createPathWithParams } = usePaneRouter(), selectedCommentId = params?.comment, paramsRef = React.useRef(params);
  ...
  return enabled ? /* @__PURE__ */ jsxRuntime.jsx(
    sanity.CommentsProvider,
    {
      ...
      documentType,
      getCommentLink,
      isCommentsOpen: inspector?.name === sanity.COMMENTS_INSPECTOR_NAME,
...
function useDocumentTitle() {
  ...
  return connectionState === "connecting" ? { error: void 0, title: void 0 } : title ? { error: void 0, title } : documentValue ? error ? { error: `Error: ${error.message}`, title: void 0 } : { error: void 0, title: value?.title } : { error: void 0, title: `New ${schemaType?.title || schemaType?.name}` };
}
const IGNORE_OPS = ["patch", "commit"], DocumentOperationResults = React.memo(function() {
  const { push: pushToast } = ui.useToast(), { documentId, documentType, value: documentPaneValue } = useDocumentPane(), documentTitleInfo = useDocumentTitle(), titleError = documentTitleInfo.error, event = sanity.useDocumentOperationEvent(documentId, documentType), prevEvent = React.useRef(event), paneRouter = usePaneRouter(), { t } = sanity.useTranslation(structureLocaleNamespace), documentTitleBase = React.useMemo(() => !documentTitleInfo.title && !titleError && !IGNORE_OPS.includes(event?.op) && typeof documentPaneValue.title == "string" && event?.type === "success" ? documentPaneValue.title : documentTitleInfo.title, [documentTitleInfo.title, titleError, event, documentPaneValue.title]) || t("panes.document-operation-results.operation-undefined-title"), documentTitle = documentTitleBase.length > 25 ? `${documentTitleBase.slice(0, 25)}...` : documentTitleBase;
  return React.useEffect(() => {
...
function PermissionCheckBanner({ granted, requiredPermission }) {
  ...
  const roleTitles = (currentUser?.roles || []).map((role) => role.title), roles = listFormat.formatToParts(roleTitles).map(
...
const ReferenceChangedBanner = React.memo(() => {
        ...
        operators.map(
          ({ draft, published }) => ({
            ...
            result: {
              ...
              refValue: PathUtils.get(draft.snapshot || published.snapshot, parentRefPath)?._ref
            }
          })
        )
      )
  ...
  return (
    ...
    referenceInfo.result?.refValue === routerReferenceId ? null : /* @__PURE__ */ jsxRuntime.jsx(
      Banner,
      {
        action: referenceInfo.result?.refValue ? {
        ...
        content: /* @__PURE__ */ jsxRuntime.jsx(ui.Text, { size: 1, weight: "medium", children: referenceInfo.result?.refValue ? t("banners.reference-changed-banner.reason-changed.text") : t("banners.reference-changed-banner.reason-removed.text") }),
...
`, FormHeader = ({ documentId, schemaType, title }) => {
  ...
  return schemaType.__experimental_formPreviewTitle === !1 ? null : /* @__PURE__ */ jsxRuntime.jsxs(TitleContainer, { marginBottom: 6, space: 4, children: [
    !isSingleton && /* @__PURE__ */ jsxRuntime.jsx(ui.Text, { muted: !0, size: 1, children: schemaType.title ?? schemaType.name }),
    /* @__PURE__ */ jsxRuntime.jsx(ui.Heading, { as: "h2", "data-heading": !0, muted: !title, "data-testid": "document-panel-document-title", children: title ?? t("document-view.form-view.form-title-fallback") })
  ] });
}, LONG_ENOUGH_BUT_NOT_TOO_LONG = 1e3 * 60 * 60 * 24 * 24;
function useConditionalToast(params) {
  const toast = ui.useToast();
  React.useEffect(() => (params.enabled && toast.push({ ...params, duration: LONG_ENOUGH_BUT_NOT_TOO_LONG }), () => {
...
const preventDefault = (ev) => ev.preventDefault(), FormView = React.forwardRef(function(props, ref) {
  const { hidden, margins } = props, {
    ...
    openPath
  } = useDocumentPane(), documentStore = sanity.useDocumentStore(), presence = sanity.useDocumentPresence(documentId), { title } = useDocumentTitle(), patchChannel = React.useMemo(() => sanity.createPatchChannel(), []), isLocked = editState?.transactionSyncLock?.enabled, { t } = sanity.useTranslation(structureLocaleNamespace), conditionalToastParams = React.useMemo(
    () => ({
      id: "sync-lock",
      status: "warning",
      enabled: isLocked,
  ...
  useConditionalToast(conditionalToastParams), React.useEffect(() => {
...
function ConfirmDialogContent(props) {
  const { dialog } = props, {
    ...
    onConfirm,
    tone
  } = dialog, { t } = sanity.useTranslation(structureLocaleNamespace), { isTopLayer } = ui.useLayer(), ref = React.useRef(null), handleGlobalKeyDown = React.useCallback(
    (event) => {
      event.key === "Escape" && isTopLayer && onCancel();
    },
    [isTopLayer, onCancel]
  );
  return ui.useGlobalKeyDown(handleGlobalKeyDown), ui.useClickOutsideEvent(isTopLayer && onCancel, () => [ref.current]), /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { direction: "column", ref, style: { minWidth: 304, maxWidth: 400 }, children: [
...
function PopoverDialogContent(props) {
  const { dialog } = props, { content, onClose } = dialog, { isTopLayer } = ui.useLayer(), ref = React.useRef(null), handleGlobalKeyDown = React.useCallback(
    (event) => {
      event.key === "Escape" && isTopLayer && onClose();
    },
    [isTopLayer, onClose]
  );
  return ui.useGlobalKeyDown(handleGlobalKeyDown), ui.useClickOutsideEvent(isTopLayer && onClose, () => [ref.current]), /* @__PURE__ */ jsxRuntime.jsx("div", { ref, children: content });
}
function DocumentActionPortalProvider(props) {
  const { children } = props, { element, elements } = ui.usePortal(), portalElement = elements?.[DOCUMENT_PANEL_PORTAL_ELEMENT] || element;
...
function ActionStateDialog(props) {
  ...
  if (dialog.type === "custom")
    return /* @__PURE__ */ jsxRuntime.jsx(DocumentActionPortalProvider, { children: dialog?.component });
...
const ActionDialogWrapper = React.memo(function({
  actionStates,
  children,
  referenceElement
}) {
  const [actionIndex, setActionIndex] = React.useState(-1), currentAction = React.useMemo(() => actionStates[actionIndex], [actionIndex, actionStates]), handleAction = React.useCallback((idx) => {
    setActionIndex(idx);
  }, []), result = React.useMemo(() => children({ handleAction }), [children, handleAction]);
  return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
...
function ActionMenuButton(props) {
  const { actionStates, disabled } = props, idPrefix = React.useId(), [referenceElement, setReferenceElement] = React.useState(null), popoverProps = React.useMemo(
    ...
    []
  ), { t } = sanity.useTranslation(structureLocaleNamespace), renderActionDialog = React.useCallback(
    ({ handleAction }) => /* @__PURE__ */ jsxRuntime.jsx(
      TooltipDelayGroupProvider.MenuButton,
      {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(ActionDialogWrapper, { actionStates, referenceElement, children: renderActionDialog });
}
...
const DISABLED_REASON_KEY$2 = {
  ...
  return React.useMemo(() => !published || liveEdit ? null : !isPermissionsLoading && !permissions?.granted ? {
    tone: "critical",
...
const HistoryRestoreAction = ({ id, type, revision, onComplete }) => {
  ...
  } : null, [handleConfirm, isConfirmDialogOpen, onComplete, t]), isRevisionInitial = revision === "@initial", isRevisionLatest = revision === void 0;
  return React.useMemo(() => isRevisionLatest ? null : {
...
const PublishAction = (props) => {
  ...
  return React.useMemo(() => {
    ...
    const disabled = !!(publishScheduled || editState?.transactionSyncLock?.enabled || publishState === "publishing" || publishState === "published" || hasValidationErrors || publish.disabled);
    return {
...
const DISABLED_REASON_KEY = {
  ...
  return React.useMemo(() => liveEdit ? null : !isPermissionsLoading && !permissions?.granted ? {
    tone: "critical",
...
const DocumentStatusBarActionsInner = React.memo(function(props) {
  ...
  const { actions: allActions, connectionState, documentId, editState } = useDocumentPane(), actions = React.useMemo(
    ...
    [allActions]
  ), renderDocumentStatusBarActions = React.useCallback(
    ({ states }) => /* @__PURE__ */ jsxRuntime.jsx(
  ...
  return actions.length === 0 || !editState ? null : /* @__PURE__ */ jsxRuntime.jsx(
    RenderActionCollectionState,
    {
      ...
      group: "default",
      children: renderDocumentStatusBarActions
    }
  );
}), HistoryStatusBarActions = React.memo(function() {
  const { actions, connectionState, editState, timelineStore } = useDocumentPane(), revision = sanity.useTimelineSelector(timelineStore, (state2) => state2.revTime)?.id || "", disabled = (editState?.draft || editState?.published || {})._rev === revision, actionProps = React.useMemo(() => ({ ...editState || {}, revision }), [editState, revision]), historyActions = React.useMemo(() => (actions ?? []).filter(isRestoreAction).slice(-1), [actions]), renderDocumentStatusBarActions = React.useCallback(
    ({ states }) => /* @__PURE__ */ jsxRuntime.jsx(
      DocumentStatusBarActionsInner,
      {
...
function DocumentHeaderTabs() {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(ui.TabList, { space: 1, children: views2.map((view, index) => /* @__PURE__ */ jsxRuntime.jsx(
    DocumentHeaderTab,
    {
      icon: view.icon,
...
function DocumentHeaderTitle() {
  ...
  return connectionState === "connecting" ? /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, {}) : title ? /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children: title }) : documentValue ? error ? /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children: t("panes.document-header-title.error.text", { error: error.message }) }) : /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children: value?.title || /* @__PURE__ */ jsxRuntime.jsx("span", { style: { color: "var(--card-muted-fg-color)" }, children: t("panes.document-header-title.untitled.text") }) }) : /* @__PURE__ */ jsxRuntime.jsx(jsxRuntime.Fragment, { children: t("panes.document-header-title.new.text", {
...
const DocumentPanelHeader = React.memo(
...
), DocumentPanelHeaderActionDialogDeferred = React.memo(
  function(props) {
    const { setReferenceElement, referenceElement, contextMenuNodes } = props, states = React.useDeferredValue(props.states);
    return /* @__PURE__ */ jsxRuntime.jsx(
  ...
  const { states, setReferenceElement, contextMenuNodes, referenceElement } = props, renderActionDialog = React.useCallback(
    ({ handleAction }) => /* @__PURE__ */ jsxRuntime.jsx("div", { ref: setReferenceElement, children: /* @__PURE__ */ jsxRuntime.jsx(
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(ActionDialogWrapper, { actionStates: states, referenceElement, children: renderActionDialog });
}), BADGE_TONES = {
  primary: "primary",
  success: "positive",
  warning: "caution",
  danger: "critical"
}, DocumentBadgesInner = React.memo(function({ states }) {
  ...
  const states = React.useDeferredValue(props.states);
  return /* @__PURE__ */ jsxRuntime.jsx(DocumentBadgesInner, { states });
});
function DocumentBadges() {
  const { badges, editState } = useDocumentPane(), renderDocumentBadges = React.useCallback(({ states }) => /* @__PURE__ */ jsxRuntime.jsx(DocumentBadgesDeferred, { states }), []);
  return !editState || !badges ? null : /* @__PURE__ */ jsxRuntime.jsx(RenderBadgeCollectionState, { badges, badgeProps: editState, children: renderDocumentBadges });
}
...
const STATUS_DICTIONARY = {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { align: "center", gap: 2, "data-testid": "pane-footer-document-status-pulse", children: [
...
function DocumentStatusLine({ singleLine }) {
  const { documentId, documentType, editState, value } = useDocumentPane(), [status, setStatus] = React.useState(null), syncState = sanity.useSyncState(documentId, documentType), lastUpdated = value?._updatedAt;
  return React.useEffect(() => {
    ...
    TooltipDelayGroupProvider.Tooltip,
    {
      content: /* @__PURE__ */ jsxRuntime.jsx(sanity.DocumentStatus, { absoluteDate: !0, draft: editState?.draft, published: editState?.published }),
      placement: "top",
      children: /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { align: "center", gap: 3, children: [
        /* @__PURE__ */ jsxRuntime.jsx(sanity.DocumentStatusIndicator, { draft: editState?.draft, published: editState?.published }),
        /* @__PURE__ */ jsxRuntime.jsx(
...
}), DocumentActionShortcuts = React.memo(
  (props) => {
    const { actionsBoxElement, as = "div", children, ...rest } = props, { actions, editState } = useDocumentPane(), [activeIndex, setActiveIndex] = React.useState(-1), onActionStart = React.useCallback((idx) => {
      ...
      [editState]
    ), renderDocumentActionShortcuts = React.useCallback(
      ({ states }) => /* @__PURE__ */ jsxRuntime.jsx(
    ...
    return !actionProps || !actions ? null : /* @__PURE__ */ jsxRuntime.jsx(RenderActionCollectionState, { actionProps, actions, children: renderDocumentActionShortcuts });
  }
);
...
function DocumentLayout() {
  ...
  const [inspectorMenuItems, setInspectorMenuItems] = React.useState([]), [rootFieldActionNodes, setRootFieldActionNodes] = React.useState([]), footerRect = ui.useElementRect(footerElement), headerRect = ui.useElementRect(headerElement), footerHeight = footerRect?.height, headerHeight = headerRect?.height, currentMinWidth = DOCUMENT_PANEL_INITIAL_MIN_WIDTH + (inspector ? DOCUMENT_INSPECTOR_MIN_WIDTH : 0), minWidth = DOCUMENT_PANEL_MIN_WIDTH + (inspector ? DOCUMENT_INSPECTOR_MIN_WIDTH : 0), currentInspector = React.useMemo(
    () => inspectors2?.find((i) => i.name === inspector?.name),
    [inspectors2, inspector?.name]
...
}), CreatedDraft = telemetry.defineEvent({
...
const DocumentPaneProvider = React.memo((props) => {
  const { children, index, pane: pane2, paneKey, onFocusPath } = props, schema = sanity.useSchema(), templates = sanity.useTemplates(), { setDocumentMeta } = sanity.useCopyPaste(), {
    __internal_tasks,
    document: {
    ...
    templateName,
    templateParams
  }), initialValue = sanity.useUnique(initialValueRaw), { patch } = sanity.useDocumentOperation(documentId, documentType), editState = sanity.useEditState(documentId, documentType), { validation: validationRaw } = sanity.useValidationStatus(documentId, documentType), connectionState = sanity.useConnectionState(documentId, documentType), schemaType = schema.get(documentType), value = editState?.draft || editState?.published || initialValue.value, [isDeleting, setIsDeleting] = React.useState(!1), actions = React.useMemo(
  ...
  ), focusPathRef = React.useRef([]), activeViewId = params.view || views2[0] && views2[0].id || null, [timelineMode, setTimelineMode] = React.useState("closed"), [timelineError, setTimelineError] = React.useState(null), timelineStore = sanity.useTimelineStore({
    documentId,
    documentType,
    onError: setTimelineError,
    rev: params.rev,
    since: params.since
  }), onOlderRevision = sanity.useTimelineSelector(timelineStore, (state2) => state2.onOlderRevision), revTime = sanity.useTimelineSelector(timelineStore, (state2) => state2.revTime), sinceAttributes = sanity.useTimelineSelector(timelineStore, (state2) => state2.sinceAttributes), timelineDisplayed = sanity.useTimelineSelector(timelineStore, (state2) => state2.timelineDisplayed), timelineReady = sanity.useTimelineSelector(timelineStore, (state2) => state2.timelineReady), isPristine = sanity.useTimelineSelector(timelineStore, (state2) => state2.isPristine), isDeleted = React.useMemo(() => timelineReady ? !editState?.draft && !editState?.published && !isPristine : !1, [editState?.draft, editState?.published, isPristine, timelineReady]), previewUrl = usePreviewUrl(value), [presence, setPresence] = React.useState([]);
  React.useEffect(() => {
  ...
  const currentInspector = inspectors2?.find((i) => i.name === inspectorName), resolvedChangesInspector = inspectors2.find((i) => i.name === HISTORY_INSPECTOR_NAME), changesOpen = currentInspector?.name === HISTORY_INSPECTOR_NAME, { t } = sanity.useTranslation(structureLocaleNamespace), inspectOpen = params.inspect === "on", compareValue = changesOpen ? sinceAttributes : editState?.published || null, fieldActions = React.useMemo(
    ...
    (blurredPath) => {
      disableBlurRef.current || (setFocusPath(sanity.EMPTY_ARRAY), focusPathRef.current !== sanity.EMPTY_ARRAY && (focusPathRef.current = sanity.EMPTY_ARRAY, onFocusPath?.(sanity.EMPTY_ARRAY)));
    },
  ...
  patchRef.current = (event) => {
    !editState.draft && !editState.published && telemetry2.log(CreatedDraft), patch.execute(sanity.toMutationPatches(event.patches), initialValue.value);
  };
  const handleChange = React.useCallback((event) => patchRef.current(event), []), closeInspector = React.useCallback(
    ...
    (nextInspectorName, paneParams) => {
      ...
      currentInspector && (currentParams = (nextInspector.onClose?.({ params: currentParams }) ?? {
  ...
  ), telemetry2 = react.useTelemetry(), handleMenuAction = React.useCallback(
    (item) => {
      if (item.action === "production-preview" && previewUrl)
        return window.open(previewUrl), !0;
      if (item.action === "copy-document-url" && navigator)
        return telemetry2.log(DocumentURLCopied), navigator.clipboard.writeText(window.location.toString()), pushToast({
    ...
    []
  ), requiredPermission = value._createdAt ? "update" : "create", liveEdit = !!schemaType?.liveEdit, docId = value._id ? value._id : "dummy-id", docPermissionsInput = React.useMemo(() => ({
    ...value,
  ...
  }), isNonExistent = !value?._id, readOnly = React.useMemo(() => {
    const hasNoPermission = !isPermissionsLoading && !permissions?.granted, updateActionDisabled = !_internal.isActionEnabled(schemaType, "update"), createActionDisabled = isNonExistent && !_internal.isActionEnabled(schemaType, "create"), reconnecting = connectionState === "reconnecting", isLocked = editState.transactionSyncLock?.enabled;
  ...
  const setOpenPath = React.useCallback(
    ...
    (nextFocusPath, payload) => {
      setFocusPath(nextFocusPath), deepEquals__default.default(focusPathRef.current, nextFocusPath) || (setOpenPath(nextFocusPath.slice(0, -1)), focusPathRef.current = nextFocusPath, onFocusPath?.(nextFocusPath)), updatePresenceThrottled(nextFocusPath, payload);
    },
  ...
  return React.useEffect(() => {
    if (ready && params.path) {
      const { path, ...restParams } = params, pathFromUrl = PathUtils.resolveKeyedPath(formStateRef.current?.value, PathUtils.fromString(path));
      disableBlurRef.current = !0, deepEquals__default.default(focusPathRef.current, pathFromUrl) || (setFocusPath(pathFromUrl), setOpenPath(pathFromUrl), focusPathRef.current = pathFromUrl, onFocusPath?.(pathFromUrl));
...
const DocumentPane = React.memo(function(props) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(sanity.SourceProvider, { name: props.pane.source || parentSourceName, children: /* @__PURE__ */ jsxRuntime.jsx(sanity.CopyPasteProvider, { children: /* @__PURE__ */ jsxRuntime.jsx(DocumentPaneInner, { ...props }) }) });
});
...
function usePaneOptions(options, params = {}) {
  ...
  return React.useMemo(() => {
    if (options.type && options.type !== "*")
      return options;
    const templateName = options.template || params.template, documentType = (templateName ? templates.find((t) => t.id === templateName) : void 0)?.schemaType;
...
function listenSearchQuery(options) {
  const {
    ...
    maxFieldDepth,
    enableLegacySearch
  } = options, sortBy = sort.by, extendedProjection = sort?.extendedProjection, events$ = rxjs.defer(() => client.listen(`*[${filter}]`, params, {
...
function useDocumentList(opts) {
  const { filter, params: paramsProp, sortOrder, searchQuery, apiVersion } = opts, client = sanity.useClient({
  ...
  }), { enableLegacySearch = !1 } = sanity.useWorkspace().search, schema = sanity.useSchema(), maxFieldDepth = sanity.useSearchMaxFieldDepth(), [resultState, setResult] = React.useState(INITIAL_STATE), { onRetry, error, result } = resultState, documents = result?.documents, items = React.useMemo(
    ...
    [filter, paramsProp]
  ), isLoading = result === null && !error, hasMaxItems = documents?.length === FULL_LIST_LIMIT, onListChange = React.useCallback(() => {
    ...
    (res) => {
      ...
      if (!res.error && res?.result === null && shouldFetchFullList) {
        setIsLazyLoading(!0);
        return;
      }
      if (documentsLength < PARTIAL_PAGE_LIMIT && documentsLength !== 0 && !shouldFetchFullList && setHasFullList(!0), res?.result === null) {
...
function DocumentSheetListProvider({
  ...
  const [selectedAnchorCellDetails, setSelectedAnchorCellDetails] = React.useState(null), [selectedRangeCellIndexes, setSelectedRangeCellIndexes] = React.useState([]), clearAndSetFocusSelection = React.useCallback(
    ...
    (event) => {
      selectedAnchorCellDetails && (document.getElementById(
        `cell-${selectedAnchorCellDetails.colId}-${selectedAnchorCellDetails.rowIndex}`
      )?.contains(event.target) || resetFocusSelection());
    },
  ...
  const focusAnchorCell = React.useCallback(
    ...
    (colId, rowIndex) => selectedAnchorCellDetails?.colId !== colId ? null : selectedAnchorCellDetails.rowIndex === rowIndex ? selectedAnchorCellDetails.state === "focused" ? "focused" : "selectedAnchor" : selectedRangeCellIndexes.includes(rowIndex) ? "selectedRange" : null,
...
function ColumnsControl({ table }) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    TooltipDelayGroupProvider.MenuButton,
    {
      button: /* @__PURE__ */ jsxRuntime.jsx(TooltipDelayGroupProvider.Button, { mode: "bleed", text: t("sheet-list.edit-columns") }),
      id: "columns-control",
      menu: /* @__PURE__ */ jsxRuntime.jsx(ui.Menu, { padding: 3, paddingTop: 4, style: { width: 240 }, children: /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { direction: "column", height: "fill", gap: 3, children: [
        ...
        /* @__PURE__ */ jsxRuntime.jsx(ui.Flex, { style: { flex: "1 1 auto", maxHeight: 320, overflowY: "scroll" }, children: /* @__PURE__ */ jsxRuntime.jsx(ui.Stack, { children: table.getAllLeafColumns().filter((column) => column.getCanHide()).map((column) => /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { marginY: 2, align: "center", children: [
          /* @__PURE__ */ jsxRuntime.jsx(
            ui.Checkbox,
            {
          ...
          /* @__PURE__ */ jsxRuntime.jsx(ui.Box, { flex: 1, paddingLeft: 3, children: /* @__PURE__ */ jsxRuntime.jsx(ui.Text, { size: 1, children: /* @__PURE__ */ jsxRuntime.jsx("label", { htmlFor: `col-visibility-${column.id}`, children: column.columnDef.header?.toString() }) }) })
        ] }, column.id)) }) }),
        /* @__PURE__ */ jsxRuntime.jsx(
          TooltipDelayGroupProvider.Button,
          {
...
const PaneContainer = React.memo(function(props) {
  const { index, isSelected, pane: pane2, paneKey } = props, { name: parentSourceName } = sanity.useSource(), {
    ...
    defaultLayout
  ), { title } = sanity.useI18nText(pane2), defaultSortOrder = React.useMemo(() => defaultOrdering?.length > 0 ? { by: defaultOrdering } : DEFAULT_ORDERING, [defaultOrdering]), [sortOrderRaw, setSortOrder] = useStructureToolSetting(
    "sort-order",
    typeName ?? pane2.id,
    //pane.id for anything that is not documentTypeList
    defaultSortOrder
...
function menuItemsFromInitialValueTemplateItems(context, templateItems) {
  ...
  return templateItems.map((item) => {
    ...
    let builder = new MenuItemBuilder(context).title(title).icon(template && template.icon || schemaType?.icon || icons.AddIcon).intent({ type: "create", params: intentParams });
...
class DocumentListBuilder extends GenericListBuilder {
  ...
  serialize(options = { path: [] }) {
    ...
    return this.spec.options?.filter !== "_type == $type" && this.spec.options.filter && !this.spec.options.apiVersion && console.warn(
...
function createStructureBuilder({
  ...
  const configContext = sanity.getConfigContextFromSource(source), context = {
    ...
    resolveDocumentNode: (options) => {
      let builder = defaultDocumentNode?.(structureBuilder, { ...options, ...configContext }) || new DocumentBuilder(context);
...
function getIntentState(intent, params, routerState, payload) {
  const panes = routerState?.panes || [], activePanes = state.activePanes || [], editDocumentId = params.id || uuid.uuid(), isTemplate = intent === "create" && params.template;
